...
impl AccountSettings {
    ...
    fn add_account(&mut self) -> Result<()> {
        ...
        // 输入账户组名称
        let account_name: String = Input::with_theme(&self.theme)
        ...
        if !account_name.chars().all(|c| c.is_ascii_alphanumeric() || c == '_') {
            self.term.write_line("只能包含字母、数字和下划线")?;
            self.term.read_line()?;
            return Ok(());
        }
        if self.config.account.iter().any(|a| a.account_name == account_name) {
    ...
    fn modify_account(&mut self) -> Result<()> {
        ...
        let new_account_name: String = Input::with_theme(&self.theme)
            .with_prompt("请输入新的账户组名称")
            .default(current.account_name.clone())
            .interact_text()?;

        if new_account_name != current.account_name {
            if self.config.account.iter().any(|a| a.account_name == new_account_name) {
                self.term.write_line("已有该账户组名称！")?;
                self.term.read_line()?;
                return Ok(());
            }
        ...
        let new_email: String = loop {
            let email: String = Input::with_theme(&self.theme)
                .with_prompt("请输入新的邮箱")
                .default(current.x_email.clone())
                .interact_text()?;
            if self.email_regex.is_match(&email) {
                break email;
            }
        ...
        let new_zone_id: String = Input::with_theme(&self.theme)
            .with_prompt("请输入新的区域ID")
            .default(current.zone_id.clone())
            .interact_text()?;

        let new_api_key: String = Input::with_theme(&self.theme)
            .with_prompt("请输入新的API Key")
            .default(current.api_key.clone())
            .interact_text()?;

        let account = &mut self.config.account[selection];
        account.account_name = new_account_name;
        account.x_email = new_email;
        account.zone_id = new_zone_id;
