...
impl PushService {
    ...
    fn process_csv_file(&self, csvfile: &str, hostnames: &str, v4_num: u32, v6_num: u32, ip_type: &str) -> Result<String> {
        ...
        // 获取IP地址、延迟、速度和数据中心信息
        let csv_data: Vec<Vec<String>> = lines.iter()
            .skip(1)
        ...
        let ips: Vec<String> = csv_data.iter()
            .map(|fields| fields[0].clone())
            .collect();
        
        let latency: Vec<String> = csv_data.iter()
            .map(|fields| format!("{} ms", fields[4]))
            .collect();
        
        let speed: Vec<String> = csv_data.iter()
            .filter_map(|fields| {
        ...
        let datacenter: Vec<String> = csv_data.iter()
            .map(|fields| fields[6].clone())
            .collect();
    ...
    fn curl_post_json(&self, url: &str, json_data: &str, headers: &[&str], timeout: u64) -> Result<(bool, String)> {
    ...
    fn curl_post_form(&self, url: &str, form_data: &str, headers: &[&str], timeout: u64) -> Result<(bool, String)> {
        let mut cmd = Command::new("curl");
        cmd.arg("-s")
           .arg("--max-time")
           .arg(timeout.to_string())
    ...
    fn curl_put_json(&self, url: &str, json_data: &str, headers: &[&str], timeout: u64) -> Result<(bool, String)> {
        let mut cmd = Command::new("curl");
        cmd.arg("-s")
           .arg("--max-time")
           .arg(timeout.to_string())
        ...
        if let Some(push_configs) = &self.config.push {
            for config in push_configs {
                if config.push_name == "企业微信" {
                    ...
                    if let (Some(corpid), Some(secret), Some(agentid), Some(userid)) = (corpid, secret, agentid, userid) {
                        // 获取access_token
                        let token_url = format!("https://qyapi.weixin.qq.com/cgi-bin/gettoken?corpid={}&corpsecret={}", corpid, secret);
                        
                        let (success, response_text) = self.curl_get(
                            &token_url,
                            &[],
                            20
                        )?;
                        ...
                        if success {
                            if let Ok(json) = serde_json::from_str::<Value>(&response_text) {
                                if json["errcode"].as_i64().unwrap_or(-1) == 0 {
                                    ...
                                    let (send_success, send_response_text) = self.curl_post_json(
                                        &send_url,
                                        &json_data,
                                        &[],
                                        20
                                    ...
                                    if send_success {
                                        if let Ok(send_json) = serde_json::from_str::<Value>(&send_response_text) {
                                            match send_json["errcode"].as_i64().unwrap_or(-1) {
                                                0 => println!("企业微信推送成功"),
                                                81013 => println!("企业微信 USERID 填写错误，请检查后重试"),
                                                60020 => println!("企业微信应用未配置本机IP地址，请在企业微信后台添加IP白名单"),
                                                _ => {
                                                    let errmsg = send_json["errmsg"].as_str().unwrap_or("未知错误");
                                    ...
                                    println!("access_token 获取失败，请检查 CORPID 和 SECRET: {}", errmsg);
                                }
                            }
                        } else {
            ...
            for config in github_push_configs {
                if config.ddns_push == ddns_name {
                    ...
                    if let Some(captures) = re.captures(&config.file_url) {
                        ...
                        // 去除空行和重复行
                        let lines: Vec<&str> = processed_ips.lines().filter(|line| !line.is_empty()).collect();
                        let unique_lines: Vec<&str> = lines.iter().cloned().collect::<std::collections::HashSet<_>>().into_iter().collect();
                        ...
                        let (check_success, check_response_text) = self.curl_get(
                            &check_url,
                            &[&auth_header],
                            20
                        )?;

                        if !check_success || check_response_text.contains("\"message\":\"Not Found\"") || check_response_text.contains("\"message\": \"Not Found\"") {
                            // 文件不存在，创建新文件
                            let create_url = format!("https://api.github.com/repos/{}/contents/{}", repo, remote_path);
                            ...
                            // 文件存在，更新文件
                            if let Ok(check_json) = serde_json::from_str::<Value>(&check_response_text) {
                                ...
                                // 从现有内容中过滤掉当前记录
                                let filtered_content = self.filter_github_content(&existing_content, &config)?;
                                ...
                                let update_url = format!("https://api.github.com/repos/{}/contents/{}", repo, remote_path);
